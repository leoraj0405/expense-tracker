      ...
      <div className={isRefreshing ? 'et-dashboard-refreshing' : undefined}>
        ...
        <div className="et-content-grid">
          <Panel
            ...
            action={
              <Link
                to={`/expense?startDate=${monthRange.startDate}&endDate=${monthRange.endDate}`}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      {groups.map((group) => (
        <Link
          ...
          to={`/group/groupexpense?grpid=${group.id || group._id}&grpname=${encodeURIComponent(group.name)}`}
          style={{
            display: 'flex',
            alignItems: 'center',
