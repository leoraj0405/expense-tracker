...
function preserveGroupQuery(search: SearchParamsLike): string {
...
function groupName(search: SearchParamsLike): string {
  const raw = search.get('grpname');
  return raw ? decodeURIComponent(raw) : 'Group';
}

function withSearch(path: string, search: SearchParamsLike): string {
  const qs = search.toString();
  return qs ? `${path}?${qs}` : path;
}

function groupSection(search: SearchParamsLike): BreadcrumbItem[] {
