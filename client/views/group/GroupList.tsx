        ...
        <tr key={groupId || index}>
          ...
          <td data-label="Actions" className="et-td-actions">
            <div className="et-actions-wrap">
              <Link
                href={`/group/groupmember?grpid=${groupId}&grpname=${group.name}&leader=${getEntityId(group.createdBy)}`}
                className="et-btn et-btn-ghost et-btn-sm"
              >
                {isOwner ? 'Members' : 'View'}
              </Link>
              <Link
                href={`/group/groupexpense?grpid=${groupId}&grpname=${group.name}&leader=${getEntityId(group.createdBy)}`}
                className="et-btn et-btn-ghost et-btn-sm"
              >
                Expenses
              </Link>
              <Link
                href={`/group/settlement?grpid=${groupId}&grpname=${group.name}`}
                className="et-btn et-btn-ghost et-btn-sm"
              >
                Settle
              </Link>
                ...
                <Menu shadow="md" width={160}>
                  ...
                  <Menu.Dropdown>
                    <Menu.Item
                      ...
                      component={Link}
                      href={`/group/editgroup?mode=edit&group=${groupId}`}
                    >
                      Edit
                    </Menu.Item>
                    <Menu.Item
  ...
  return (
    <AppShellLayout>
      <PageHero
        ...
        action={
          <Link href="/group/addgroup" className="et-btn et-btn-primary">
